...
if __name__ == "__main__":
    ...
    testing_data = [
        ('aprehender a escribir bien es dificil', {'aprehender': 'aprender'}),
        ('EL ladron fue aprendido el dia de hoy', {'aprendido': 'arrestado'}),
        ('aprender a manejar es facil cuando tienes un auto propio', {}),
        ('los rebeldes fueron aprehendidos el dia jueves', {}),
        ('Ayer fue deshecho la banda criminal mas peligrosa de la zona', {}),
        ('EL dia hoy fue desecho la banda criminal mas peligrosa de la zona', {'desecho': 'detenida'}),
        ('La empresa minera lanzaba al rio el desecho de sus trabajadores', {'desecho': 'detenida'}),
        ('La empresa minera lanzaba al lago sus desechos residuales', {})
    ]
    ...
    for data in testing_data:
        doc = nlp(data[0])
        print("="*50)
        if {k.text: doc._.suggestions_spellCheck[k] for k in doc._.suggestions_spellCheck.keys() if k.text != doc._.suggestions_spellCheck[k]} == data[1]:
            print("ASSERT TRUE")
        else:
